...
The greenhouse climate model integrates the models of temperature, humidity, and \coo\ concentration into a dynamic system represented by a state vector \(\mathbf{z}\) = [\(T_c, T_i, T_v, T_m, T_p, T_f, T_s, C_w, C_c, x_{\text{sdw}}, x_{\text{nsdw}}\)] with dimension
% RP: Znovu opakujem, ze takto sa rozmery neuvadzaju. z\in\mathbb R^11 je urcite lepsi sposob.
(11, 1), and input vector \(\mathbf{u}\) = [\(Q_{\text{heater}}, R_{\text{fan}}, V_{\text{humid}}, G_{\coo} \)] with dimension (4, 1) for the heating power, ventilation, humidification, and \coo\ enrichment. The \(T_c\) represents the cover temperature, \(T_i\) the internal air temperature, \(T_v\) the plant temperature, \(T_m\) the growing medium temperature, \(T_p\) the tray temperature, \(T_f\) the floor temperature, and \(T_s\) the temperature of the soil layer. Additionally, \(C_w\) denotes the density of water vapor, \(C_c\) the \coo\ density, \(x_{\text{sdw}}\) the structural dry weight of the plant, and \(x_{\text{nsdw}}\) the non-structural dry weight of the plant.
...
% RP: salat sa tu zjavi z cista jasna. hodilo by sa ho tu uviest.
The growth of lettuce model captures the dynamic behavior of both structural (SDW) and non-structural dry weight (NSDW) of the plant~\cite{VANHENTEN199455}. The dynamics is influenced by environmental factors such as temperature, light (PAR), and \coo\ concentration. The model is parameterized using well-established constants from the literature. For the complete list of parameters and their values with references, see the source code repository\footnote{\url{https://github.com/MarekWadinger/dynamic_opt_growth_model/blob/main/core/lettuce_model.py}}.
...
% RP: ako recenzent absolutne neznasam necislovane rovnice. neviem sa vtedy odkazat na rovnicu ak ju chcem komentovat. akoby to tu niekto nedokoncil. Bolo by tiez fajn naznacit zavislost jednotlivych casti na ostatnych stavoch. nieco ako "\ui{r}{gr}(T)"
\[
...
% RP: Treba zjednotit kapitalizaciu nazvov. Teraz je to mix.
\label{sec:mpc}
...
\begin{figure}\label{fig:web}
    ...
    % RP: Tiene mozu ostat, hoci mne sa nepacia. Neda sa aspon nieco spravit s bielym miestom, najma na hornom a dolnom okraji? To iba zbyticne zabera miesto v clanku.
...
% RP: Ake numericke nastroje boli pouzite na optimalizaciu/simulaciu? Ak napr. CasADi, tak by malo byt referencovane. 
First, we analyze the nonlinear behavior of the greenhouse under varying climate and actuation conditions. We observed how the actuators influence the growth of SDW and NSDW. The simulations were run under a mild climate of Bratislava (Slovakia) on the day of 11\textsuperscript{th} October. Figure~\ref{fig:steps} shows that the influence of ventilation 
% RP: tu by sme mali citatela nasmerovat na spravny stlpec in obrazku. inak nema sancu.
and humidification is insignificant on growth as compared to no change in actuation applied.
Nevertheless, there is a positive effect on convection and the overall transfer of energy. Intense heating positively influences the conversion of NSDW into SDW, yet it does not influence the overall NSDW buildup. \coo\ enrichment has a significant impact on non-structural plant growth.
