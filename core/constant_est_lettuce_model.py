...
def lettuce_growth_model_ce(
    _: int, x: tuple[float, float], u: tuple[float, float, float], c: tuple[float, float, float, float, float] = None
) -> tuple[float, float]:
    if c is None:
        # Assign default values for u if it's not provided
    ...
    Args:
        ...
        c (tuple): System parameters [c_yf, c_gr_max, c_k, c_lar, c_epsilon] (optional)
    ...
    global C_YF, C_GR_MAX, C_K, C_LAR, C_EPSILON
    C_YF = c[0]
    C_GR_MAX = c[1]
    C_K = c[2]
    C_LAR = c[3]
    ...
    dx_sdw_dt, dx_nsdw_dt, __ = _lettuce_growth_model_ce(_, x, u)
    return dx_sdw_dt, dx_nsdw_dt


def _lettuce_growth_model_ce(
