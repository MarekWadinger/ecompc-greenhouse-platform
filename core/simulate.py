...
def gen_sine_u(
    ...
    """
    t = t / (sim_time - 1)  # Normalize t to the range [0, 1]

    u = tuple()
    for u_min_, u_max_ in zip(u_min, u_max):
        # Calculate the values of each sine wave at time t
        u_ = u_min_ + (u_max_ - u_min_) / 2 * (1 + np.sin(2 * np.pi * t))
        u += (u_,)
    return u
...
def simulate_open_loop(
    ...
    for t in range(sim_time):
        ...
        x_ode = solve_ivp(system, tspan, x_ode, args=(u,), method="RK45")
        x_ode = x_ode.y[:, -1]
...
def simulate_closed_loop(
    ...
    for t in range(sim_time):
        ...
        k = np.array([1., 1., 1.])
        u = -k @ (np.array(x_ode) - np.array(x_ref))
